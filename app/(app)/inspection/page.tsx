...
export default function InspectionPage() {
  ...
  const statusColor: Record<string,string> = { Good:'#4ade80', 'Needs Attention':'#fbbf24', Critical:'#f87171', Pending:'#6b7280' }

  const summaryRows: { label:string; bg:string; tc:string; n:number }[] = [
    { label:'Good',       bg:'#22c55e', tc:'#4ade80', n:goodCount },
    { label:'Needs Attn', bg:'#eab308', tc:'#fbbf24', n:okCount  },
    { label:'Issues',     bg:'#ef4444', tc:'#f87171', n:badCount  },
  ]
          ...
          <div>
            ...
            <div className="mc-card">
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:16 }}>
                {summaryRows.map(row => (
                  <div key={row.label} style={{ background:`${row.bg}14`, border:`1px solid ${row.bg}33`, borderRadius:8, padding:12, textAlign:'center' }}>
                    <div style={{ fontSize:24, fontWeight:800, color:row.tc, fontFamily:"'Syne',sans-serif" }}>{row.n}</div>
                    <div style={{ fontSize:11, color:'#4b5563', marginTop:2 }}>{row.label}</div>
                  </div>
                ))}
              </div>
